...
def test_graph_has_expected_edge_count(graph: Graph) -> None:
    """28 current roads + 15 proposed roads = 43 edges total."""
    assert graph.edge_count == 43
...
def test_graph_has_2_parks(graph: Graph) -> None:
    parks = graph.get_facilities_by_type(NodeType.PARK)
    assert len(parks) == 2


def test_graph_has_2_commercial_nodes(graph: Graph) -> None:
    commercial = graph.get_facilities_by_type(NodeType.COMMERCIAL)
